...
//    outfile.open("mt-state.txt");
//    outfile << generator;
//    outfile.close();

//    std::ifstream infile;
//    infile.open("mt-wanted-state.txt", std::ios::binary);
//    std::array<uint32_t, 624> int_state;
//    infile.read ((char*) int_state.data(), int_state.size() * sizeof(uint32_t));
//    infile.close();
//    memcpy(generator._Ax, int_state.data(), int_state.size() * sizeof(uint32_t));
    ...
    std::cout << "10m tries in " << std::chrono::duration_cast<std::chrono::seconds>(duration).count() << " seconds" << std::endl;
//    std::cout << "Randoms: 0x" << std::hex;
//    for(int i = 0; i < 16; i++){
//        std::cout << generator() << ", 0x";
//    }
//    std::cout << std::endl;
